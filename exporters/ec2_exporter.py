...
reserved_instance_types_gauge = Gauge('aws_ec2_reserved_instance_types', 'Number of Reserved EC2 Instances by type', ['account_id', 'region', 'instance_type'])
...
def collect_reserved_instance_metrics(region='us-east-1'):
    ...
    try:
        ...
        for instance_type, count in reserved_instance_type_count.items():
            reserved_instance_types_gauge.labels(account_id=ACCOUNT_ID, region=region, instance_type=instance_type).set(count)
